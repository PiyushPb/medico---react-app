...
svc = pickle.load(open('models/drugsSuggestion.pkl','rb'))
...
@app.route('/api/v1/symptomsPrediction', methods=['POST'])
def predict_disease():
    try:
        ...
        # Convert desc, pre, and med to lists if they are numpy arrays
        ...
        # Check if wrkout is a pandas series and convert it to a list
        if isinstance(wrkout, pd.Series):
            wrkout = wrkout.tolist()

        return jsonify({'predicted_disease': predicted_disease, 'description': desc, 'precautions': pre, 'medications': med, 'diet': die, 'workout': wrkout})
    except Exception as e:
        # Handle any exceptions
        return jsonify({'error': str(e)})
